...
public class ItemModels extends ItemModelProvider {
    ...
    protected void registerModels() {
        ...
        for (RegistryEntrySupplier<Item> sup : ModItems.ITEMS.getEntries()) {
            ...
            if (ribbons.contains(sup)) {
                this.singleTexture(sup.getID().getPath(), this.mcLoc("item/generated"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath()))
                ...
                if (this.dualItemMapping.containsKey(sup))
                    this.singleTexture(sup.getID().getPath(), this.mcLoc("item/handheld"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath()))
                ...
                this.singleTexture(sup.getID().getPath(), RuneCraftory.modRes("item/hammer_tool"), "layer0", this.modLoc("item/" + sup.getID().getPath()));
            else if (sup.get() instanceof ItemToolSickle)
                this.singleTexture(sup.getID().getPath(), RuneCraftory.modRes("item/sickle"), "layer0", this.modLoc("item/" + sup.getID().getPath()));
            else if (sup.get() instanceof ItemToolWateringCan)
                this.singleTexture(sup.getID().getPath(), RuneCraftory.modRes("item/watering_can"), "layer0", this.modLoc("item/" + sup.getID().getPath()));
            else if (sup.get() instanceof ItemToolHoe)
                this.singleTexture(sup.getID().getPath(), RuneCraftory.modRes("item/hoe"), "layer0", this.modLoc("item/" + sup.getID().getPath()));
            else if (sup.get() instanceof ItemToolAxe)
                this.singleTexture(sup.getID().getPath(), RuneCraftory.modRes("item/axe_tool"), "layer0", this.modLoc("item/" + sup.getID().getPath()));
                ...
                this.singleTexture(sup.getID().getPath(), this.mcLoc("item/generated"), "layer0", RuneCraftory.modRes("item/" + ModItems.UNKNOWN.getID().getPath()));
            else if (sup.get() instanceof ItemGiantCrops)
                this.singleTexture(sup.getID().getPath(), this.modLoc("item/double_sized_item"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath()));
            else if (sup == ModItems.NPC_BABY)
                this.singleTexture(sup.getID().getPath(), this.mcLoc("item/generated"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath() + "_boy"))
                        .override().predicate(ItemModelProps.BABY_GENDER, 1)
                        .model(this.singleTexture(sup.getID().getPath() + "_girl", this.mcLoc("item/generated"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath() + "_girl"))).end();
            else
                this.singleTexture(sup.getID().getPath(), this.mcLoc("item/generated"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath()));
        }
    }
    ...
    private void createBigWeaponModel(RegistryEntrySupplier<Item> sup, ResourceLocation heldModel) {
        this.singleTexture(sup.getID().getPath(), this.mcLoc("item/handheld"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath()))
                ...
                .model(this.singleTexture(sup.getID().getPath() + "_held", heldModel, "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath() + "_held")))
                .end();
    }
    ...
    private void createGloveModels(RegistryEntrySupplier<Item> sup) {
        if (this.existingSameGloveItems.contains(sup)) {
            ...
            this.singleTexture(sup.getID().getPath(), this.mcLoc("item/handheld"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath()))
                    .override().predicate(ItemModelProps.GLOVE_HELD_ID, 0.25f)
                    .model(this.singleTexture(sup.getID().getPath() + "_held", this.modLoc("item/fist"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath() + "_held"))).end()
                    .override().predicate(ItemModelProps.GLOVE_HELD_ID, 0.5f)
                    .model(this.singleTexture(sup.getID().getPath() + "_held_left", this.modLoc("item/fist_left"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath() + "_held"))).end()
                    .override().predicate(ItemModelProps.GLOVE_HELD_ID, 0.75f)
                    .model(this.singleTexture(sup.getID().getPath() + "_held_s", this.modLoc("item/fist_s"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath() + "_held"))).end()
                    .override().predicate(ItemModelProps.GLOVE_HELD_ID, 1)
                    .model(this.singleTexture(sup.getID().getPath() + "_held_s_left", this.modLoc("item/fist_s_left"), "layer0", RuneCraftory.modRes("item/" + sup.getID().getPath() + "_held"))).end();
    }
}
