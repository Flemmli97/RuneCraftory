...
public record SpellPropertiesgen(PackOutput packOutput) implements DataProvider {
    ...
    public CompletableFuture<?> run(CachedOutput cache) {
        return CompletableFuture.allOf(ModSpells.DEFAULT_PROPERTIES.entrySet().stream().map((entry) -> {
            Path path = this.packOutput.getOutputFolder(PackOutput.Target.DATA_PACK).resolve(entry.getKey().getID().getNamespace() + "/" + SpellPropertiesManager.ID + "/" + entry.getKey().getID().getPath() + ".json");
