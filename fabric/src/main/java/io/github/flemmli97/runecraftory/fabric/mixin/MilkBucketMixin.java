...
public abstract class MilkBucketMixin {
    ...
    public boolean finishUsingItem(LivingEntity instance, Operation<Boolean> original) {
        ((EntityDataGetter) instance).runecraftory$effectCuringProcess(true);
        boolean orig = original.call(instance);
        ((EntityDataGetter) instance).runecraftory$effectCuringProcess(false);
        return orig;
    }
}
