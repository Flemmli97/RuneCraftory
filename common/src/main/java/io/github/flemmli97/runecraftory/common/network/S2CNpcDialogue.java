...
public record S2CNpcDialogue(int entity, NPCData.ConversationType type, String conversationID, Component component,
    ...
    public static S2CNpcDialogue read(FriendlyByteBuf buf) {
        return new S2CNpcDialogue(buf.readInt(), buf.readBoolean() ? buf.readEnum(NPCData.ConversationType.class) : null,
    ...
    public static void handle(S2CNpcDialogue pkt) {
        ...
        if (entity instanceof EntityNPCBase npc)
            ClientHandlers.updateNPCDialogue(npc, pkt.type, pkt.conversationID, pkt.component, pkt.actions);
    }
