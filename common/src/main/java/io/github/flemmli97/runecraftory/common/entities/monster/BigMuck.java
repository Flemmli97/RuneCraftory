...
public class BigMuck extends BaseMonster {
    ...
    public ExtendedBehaviour<? extends BaseMonster> getCooldownAI() {
        return SelectableBehaviourBuilder.<BaseMonster>builder()
                .add(3, new SetWalkTargetToAttackTarget<>(), new MoveToWalkTillClose<>())
    ...
    public void handleAttack(AnimationState anim) {
        if (anim.is(SPORE)) {
            ...
            if (anim.isAt("attack")) {
                RuneCraftorySpells.SPORE_CIRCLE_SPELL.get().use(this);
            }
        } else
    ...
    public void handleRidingCommand(int command) {
        if (!this.getAnimationHandler().hasAnimation()) {
            if (!this.getProp().rideActionCosts.canRun(command, this.getControllingPassenger(), command == 1 ? RuneCraftorySpells.SPORE_CIRCLE_SPELL.get() : null))
                return;
            if (command == 1)
