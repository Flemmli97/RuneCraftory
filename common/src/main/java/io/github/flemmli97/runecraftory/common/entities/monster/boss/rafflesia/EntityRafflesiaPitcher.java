    ...
    public static void rafflesiaSpawning(EntityRafflesiaPart part) {
        ...
        if (nearby.size() < 5) {
            ...
            for (int i = 0; i < rand; i++) {
                ...
                Optional<EntityType<?>> opt = Registry.ENTITY_TYPE.getTag(RunecraftoryTags.EntityTypes.RAFFLESIA_SUMMONS).flatMap(named -> named.getRandomElement(part.getRandom()).map(Holder::value));
                opt.ifPresent(type -> {
