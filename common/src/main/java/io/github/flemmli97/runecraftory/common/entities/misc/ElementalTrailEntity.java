...
public class ElementalTrailEntity extends BaseDamageCloud {
    ...
    public ElementalTrailEntity(Level level, LivingEntity thrower, ItemElement element) {
        super(RuneCraftoryEntities.ELEMENTAL_TRAIL.get(), level, thrower);
    ...
    protected boolean damageEntity(LivingEntity target) {
        if (CombatUtils.damageWithFaintAndCrit(this.getOwner(), target, new DynamicDamage.Builder(this, this.getOwner()).magic().noKnockback().hurtResistant(10).element(this.element), CombatUtils.getAttributeValue(this.getOwner(), RuneCraftoryAttributes.MAGIC_ATTACK.asHolder()) * this.damageMultiplier, null)) {
