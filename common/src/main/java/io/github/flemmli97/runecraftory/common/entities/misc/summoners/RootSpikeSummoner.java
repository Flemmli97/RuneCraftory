...
public class RootSpikeSummoner extends ProjectileSummonHelperEntity {
    ...
    public RootSpikeSummoner(Level level, LivingEntity caster) {
        super(RuneCraftoryEntities.ROOT_SPIKE_SUMMONER.get(), level, caster);
    ...
    protected void summonProjectiles() {
        if (this.ticksExisted % 10 != 0 || this.getOwner() == null)
            return;
        SpikeEntity spike = new SpikeEntity(this.level(), this.getOwner(), 0, 10, SpikeEntity.SpikeType.ROOT);
        Vec3 targetPos = null;
