...
public class FireballEntity extends BaseProjectile {
    ...
    public FireballEntity(Level level, LivingEntity shooter, boolean big) {
        super(RuneCraftoryEntities.FIRE_BALL.get(), level, shooter);
    ...
    protected boolean entityRayTraceHit(EntityHitResult result) {
        boolean att = CombatUtils.damageWithFaintAndCrit(this.getOwner(), result.getEntity(), new DynamicDamage.Builder(this, this.getOwner()).magic().noKnockback().element(ItemElement.FIRE).hurtResistant(4), CombatUtils.getAttributeValue(this.getOwner(), RuneCraftoryAttributes.MAGIC_ATTACK.asHolder()) * this.damageMultiplier, null);
        this.level().playSound(null, result.getEntity().blockPosition(), SoundEvents.GENERIC_EXPLODE.value(), this.getSoundSource(), 1.0f, 1.0f);
        this.discard();
        return att;
    }
