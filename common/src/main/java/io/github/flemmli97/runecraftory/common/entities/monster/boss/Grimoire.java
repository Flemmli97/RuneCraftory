...
public class Grimoire extends BossMonster {
    ...
    private static final ImmutableMap<String, BiConsumer<AnimationState, Grimoire>> ATTACK_HANDLER = createAnimationHandler(b -> {
        ...
        b.put(GUST, (anim, entity) -> {
            if (anim.isAt("attack")) {
                RuneCraftorySpells.GUST_ROCKS.get().use(entity);
            }
        });
        b.put(WIND_BREATH, (anim, entity) -> {
            if (anim.isAt("attack")) {
                RuneCraftorySpells.WIND_BLADE_BARRAGE.get().use(entity);
            }
        });
        b.put(TORNADO, (anim, entity) -> {
            if (anim.isAt("attack")) {
                RuneCraftorySpells.TORNADO.get().use(entity);
            }
        });
    ...
    public ExtendedBehaviour<? extends BaseMonster> getCombatAI() {
        return AttackBehaviourBuilder.<BossMonster>create()
                ...
                .condition(MonsterBehaviourUtils.ifCloserThan(16))
                .prepareOptional(MonsterBehaviourUtils.moveAttack())
                .end(8)
...
//                this.level().addParticle(new ColoredParticleData(RuneCraftoryParticles.WIND.get(), 67 / 255F, 163 / 255F, 65 / 255F, 1, 0.4f), this.getX(), this.getY() + 0.2, this.getZ(), vec.x(), vec.y(), vec.z());
    ...
    public void handleAttack(AnimationState anim) {
        BiConsumer<AnimationState, Grimoire> handler = ATTACK_HANDLER.get(anim.getID());
        if (handler != null)
    ...
    public void handleRidingCommand(int command) {
        if (!this.getAnimationHandler().hasAnimation()) {
            if (command == 2) {
                if (this.getProp().rideActionCosts.canRun(command, this.getControllingPassenger(), RuneCraftorySpells.WIND_BLADE_BARRAGE.get()))
