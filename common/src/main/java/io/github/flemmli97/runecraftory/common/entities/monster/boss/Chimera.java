...
public class Chimera extends BossMonster {
    ...
    private static final ImmutableMap<String, BiConsumer<AnimationState, Chimera>> ATTACK_HANDLER = createAnimationHandler(b -> {
        ...
        b.put(SLASH, (anim, entity) -> {
            if (anim.isAt("attack")) {
                RuneCraftorySpells.SLASH.get().use(entity);
            }
        });
    ...
    public RunecraftoryBossbar createBossBar() {
        return new RunecraftoryBossbar(RuneCraftoryEntities.CHIMERA.getID(), this.getDisplayName(), BossEvent.BossBarColor.BLUE, BossEvent.BossBarOverlay.PROGRESS)
                .setMusic(RuneCraftorySounds.CHIMERA_FIGHT.get());
    }
    ...
    public ExtendedBehaviour<? extends BaseMonster> getCombatAI() {
        return AttackBehaviourBuilder.<Chimera>create()
                .start(MonsterBehaviourUtils.checkedAttack(AnimationPlayHolder.<Chimera>builder(LEAP)
                        .start(BITE, m -> m.chargeAttackSuccess).build())).play(MonsterBehaviourUtils.cooldownedPlay())
                .condition(MonsterBehaviourUtils.ifCloserThan(12))
                .end(10)
                .start(MonsterBehaviourUtils.checkedAttack(FIRE_TAIL_BUBBLE)).play(MonsterBehaviourUtils.cooldownedPlay())
                .condition(MonsterBehaviourUtils.ifCloserThan(18))
                ...
                .condition(MonsterBehaviourUtils.ifCloserThan(10))
    ...
    public ExtendedBehaviour<? extends BaseMonster> getCooldownAI() {
        return SelectableBehaviourBuilder.<BaseMonster>builder()
                ...
                .add(7, MonsterBehaviourUtils.ifCloserThan(12), new SetWalkTargetAwayFromTarget<BaseMonster>().radius(4).speedMod((e, t) -> 1.1f), MonsterBehaviourUtils.moveTo()).build();
    }
    ...
    public void handleAttack(AnimationState anim) {
        BiConsumer<AnimationState, Chimera> handler = ATTACK_HANDLER.get(anim.getID());
        if (handler != null)
    ...
    public void handleRidingCommand(int command) {
        if (!this.getAnimationHandler().hasAnimation()) {
            if (command == 2) {
                if (this.getProp().rideActionCosts.canRun(command, this.getControllingPassenger(), RuneCraftorySpells.BUBBLE_BEAM.get()))
                    this.getAnimationHandler().setAnimation(BUBBLE_BEAM);
            } else if (command == 1) {
                if (this.getProp().rideActionCosts.canRun(command, this.getControllingPassenger(), RuneCraftorySpells.SLASH.get()))
