...
public class GoblinGangster extends Goblin {
    ...
    public static final AnimationsBuilder BUILDER = new AnimationsBuilder(Goblin.BUILDER, SLEEP);
    ...
    private final AnimationHandler<GoblinGangster> animationHandler = new AnimationHandler<>(this, ANIMS);
    ...
    protected void populateDefaultEquipmentSlots(RandomSource random, DifficultyInstance difficulty) {
        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(RuneCraftoryItems.CUTLASS_PROP.get()));
        this.setDropChance(EquipmentSlot.MAINHAND, 0);
        this.setItemSlot(EquipmentSlot.OFFHAND, new ItemStack(RuneCraftoryItems.CUTLASS_PROP.get()));
    ...
    public void handleRidingCommand(int command) {
        if (!this.getAnimationHandler().hasAnimation()) {
            if (!this.getProp().rideActionCosts.canRun(command, this.getControllingPassenger(), command == 1 ? RuneCraftorySpells.THROW_HAND_ITEM.get() : null))
                return;
            if (command == 1)
