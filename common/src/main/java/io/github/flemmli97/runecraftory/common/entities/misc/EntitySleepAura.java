...
public class EntitySleepAura extends BaseDamageCloud {
    ...
    protected boolean damageEntity(LivingEntity target) {
        if (CombatUtils.damageWithFaintAndCrit(this.getOwner(), target, new CustomDamage.Builder(this, this.getOwner()).hurtResistant(4).magic().element(EnumElement.EARTH).withChangedAttribute(ModAttributes.SLEEP.get(), 100), CombatUtils.getAttributeValue(this.getOwner(), ModAttributes.MAGIC_ATTACK.get()) * this.damageMultiplier, null)) {
