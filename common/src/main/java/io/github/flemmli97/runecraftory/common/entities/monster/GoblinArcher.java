...
public class GoblinArcher extends Goblin {
    ...
    private final AnimationHandler<GoblinArcher> animationHandler = new AnimationHandler<>(this, ANIMS);
    ...
    public void handleRidingCommand(int command) {
        if (!this.getAnimationHandler().hasAnimation()) {
            if (!this.getProp().rideActionCosts.canRun(command, this.getControllingPassenger(), command == 2 ? RuneCraftorySpells.TRIPLE_ARROW.get() : null))
                return;
            if (command == 2)
