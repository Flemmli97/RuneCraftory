...
public record ConfigHolder<T>(ConfigType configType, String configName,
                              T configSpec, Consumer<T> loader) {

    public static final Map<IConfigSpec, ConfigHolder<?>> CONFIGS = new LinkedHashMap<>();

    static {
        CONFIGS.put(GeneralConfigSpec.SPEC.getRight(), new ConfigHolder<>(ConfigType.COMMON, RuneCraftory.MODID + File.separator + "general.toml",
                GeneralConfigSpec.SPEC.getLeft(), ConfigHolder::loadGeneral));
        CONFIGS.put(ClientConfigSpec.SPEC.getRight(), new ConfigHolder<>(ConfigType.CLIENT, RuneCraftory.MODID + File.separator + "client.toml",
                ClientConfigSpec.SPEC.getLeft(), ConfigHolder::loadClient));
        CONFIGS.put(MobConfigSpec.SPEC.getRight(), new ConfigHolder<>(ConfigType.COMMON, RuneCraftory.MODID + File.separator + "mobs.toml",
    ...
    public static void loadGeneral(GeneralConfigSpec spec) {
        ...
        GeneralConfig.MOVE_SPEED_ATTACK.read(spec.moveSpeedAttack.get());
