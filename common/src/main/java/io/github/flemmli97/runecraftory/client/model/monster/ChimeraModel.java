...
public class ChimeraModel<T extends Chimera> extends EntityModel<T> implements ExtendedModel, RideableModel<T> {

    public static final ResourceLocation LOCATION = RuneCraftory.modRes("entity/chimera");

    private final ReloadableCache<ModelPartsContainer> model;
    protected final ReloadableCache<BedrockAnimations> anim;

    public ModelPartsContainer.ModelPartExtended upperBody;
    ...
    public ChimeraModel() {
        ...
        this.model = GeoModelManager.getInstance().getModel(LOCATION, model -> {
            this.upperBody = model.getPart("upperBody");
