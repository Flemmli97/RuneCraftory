...
public record SerializableBehaviour<T>(MapCodec<T> codec, BehaviourSequenceFactory<T> factory) {
    ...
    public record SerializabledBehaviourHolder<T>(SerializableBehaviour<T> type, T data) {
        ...
        public static final Codec<SerializabledBehaviourHolder<?>> CODEC = ModNPCBehaviour.BEHAVIOURS.registry().byNameCodec()
                .dispatch(SerializabledBehaviourHolder::type, SerializableBehaviour::holderCodec);

        public List<ExtendedBehaviour<EntityNPCBase>> create(Consumer<Predicate<EntityNPCBase>> predicate) {
    ...
    public interface BehaviourSequenceFactory<T> {
        ...
        default List<ExtendedBehaviour<EntityNPCBase>> create(T data, Consumer<Predicate<EntityNPCBase>> predicate) {
            this.addCondition(data, predicate);
            return this.create(data);
        }

        List<ExtendedBehaviour<EntityNPCBase>> create(T data);

        default void addCondition(T data, Consumer<Predicate<EntityNPCBase>> predicate) {
        }
    }
}
