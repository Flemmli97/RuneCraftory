...
import io.github.flemmli97.runecraftory.client.model.monster.GhostModel;
...
public class GhostRender<T extends Ghost> extends RenderMonster<T, GhostModel<T>> {

    public GhostRender(EntityRendererProvider.Context ctx, ResourceLocation texture) {
        super(ctx, new GhostModel<>(), texture, 0);
    }
    ...
    public boolean shouldRender(T entity, Frustum camera, double camX, double camY, double camZ) {
        ...
        if (anim != null && anim.is(Ghost.VANISH)) {
