...
public class Ambrosia extends BossMonster {
    ...
    public static final String SPAWN = BUILDER.add("spawn", AnimationsBuilder.definition(2).marker("sound", 0.4));
    ...
    private static final ImmutableMap<String, BiConsumer<AnimationState, Ambrosia>> ATTACK_HANDLER = createAnimationHandler(b -> {
        b.put(BUTTERFLY, (anim, entity) -> {
            if (anim.isAt("attack")) {
                RuneCraftorySpells.BUTTERFLY.get().use(entity);
            }
        });
        ...
        b.put(SLEEP, (anim, entity) -> {
            ...
            if (anim.isAt("attack"))
                RuneCraftorySpells.SLEEP_BALLS.get().use(entity);
        });
        ...
        b.put(SPAWN, (anim, entity) -> {
            if (anim.isAt("sound")) {
                entity.playSound(RuneCraftorySounds.ENTITY_AMBROSIA_SPAWN.get(), 1, 0.8f + (entity.getRandom().nextFloat() - entity.getRandom().nextFloat()) * 0.2f);
            }
        });
        b.put(ANGRY, (anim, entity) -> {
            if (anim.isAt("sound")) {
                entity.playSound(RuneCraftorySounds.ENTITY_AMBROSIA_ANGRY.get(), 1, 1 + (entity.getRandom().nextFloat() - entity.getRandom().nextFloat()) * 0.2f);
            }
        });
    });
    ...
    public ExtendedBehaviour<? extends BaseMonster> getCombatAI() {
        return AttackBehaviourBuilder.<Ambrosia>create()
                ...
                .condition(MonsterBehaviourUtils.ifCloserThan(24))
                .prepare(new SetWalkTargetAwayFromTarget<Ambrosia>().speedMod(1.1f).radius(8))
                ...
                .condition(MonsterBehaviourUtils.and(BossMonster::isEnraged, MonsterBehaviourUtils.ifCloserThan(10)))
                .prepare(new SetWalkTargetToAttackTarget<Ambrosia>().speedMod((e, t) -> 1.2f)
    ...
    public void handleAttack(AnimationState anim) {
        BiConsumer<AnimationState, Ambrosia> handler = ATTACK_HANDLER.get(anim.getID());
        if (handler != null)
    ...
    public void handleRidingCommand(int command) {
        if (!this.getAnimationHandler().hasAnimation()) {
            if (command == 2) {
                if (this.getProp().rideActionCosts.canRun(command, this.getControllingPassenger(), RuneCraftorySpells.WAVE.get()))
                    this.getAnimationHandler().setAnimation(WAVE);
            } else if (command == 1) {
                if (this.getProp().rideActionCosts.canRun(command, this.getControllingPassenger(), RuneCraftorySpells.SLEEP_BALLS.get()))
