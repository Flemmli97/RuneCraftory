...
public class ElementalBallEntity extends BaseProjectile {
    ...
    protected boolean entityRayTraceHit(EntityHitResult result) {
        boolean att = CombatUtils.damageWithFaintAndCrit(this.getOwner(), result.getEntity(), new DynamicDamage.Builder(this, this.getOwner()).magic().noKnockback().element(this.element).hurtResistant(5), CombatUtils.getAttributeValue(this.getOwner(), RuneCraftoryAttributes.MAGIC_ATTACK.asHolder()) * this.damageMultiplier, null);
        this.discard();
        this.playHitSound();
        return att;
    }
    ...
    protected void playHitSound() {
        if (this.element == ItemElement.WATER) {
            SoundEvent event = this.getVariant() == 1 ? SoundEvents.GENERIC_EXPLODE.value() : RuneCraftorySounds.SPELL_GENERIC_WATER.get();
