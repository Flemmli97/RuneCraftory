...
public class EntityHomingEnergyOrb extends BaseDamageCloud implements PowerableMob {
    ...
    protected boolean damageEntity(LivingEntity target) {
        return CombatUtils.damageWithFaintAndCrit(this.getOwner(), target, new DynamicDamage.Builder(this, this.getOwner()).damageType(DynamicDamage.DamageCategory.IGNOREMAGICDEF).noKnockback().hurtResistant(7).element(ItemElement.LIGHT), CombatUtils.getAttributeValue(this.getOwner(), ModAttributes.MAGIC_ATTACK.asHolder()) * this.damageMultiplier, null);
    }
