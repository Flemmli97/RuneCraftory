...
public class LeafBall extends BaseMonster {
    ...
    public ExtendedBehaviour<? extends BaseMonster> getCombatAI() {
        return AttackBehaviourBuilder.<BaseMonster>create()
                ...
                .prepare(new SetWalkTargetToAttackTarget<BaseMonster>().closeEnoughDist(MonsterBehaviourUtils.closeEnough(5))).prepareOptional(MonsterBehaviourUtils.moveAttack())
                .end(4)
                .build();
    }
    ...
    public void handleAttack(AnimationState anim) {
        if (anim.is(WIND)) {
            ...
            this.getNavigation().stop();
            if (anim.isAt("attack")) {
                RuneCraftorySpells.SLEEP_AURA.get().use(this);
            }
        } else
    ...
    public void handleRidingCommand(int command) {
        if (!this.getAnimationHandler().hasAnimation()) {
            if (!this.getProp().rideActionCosts.canRun(command, this.getControllingPassenger(), command == 0 ? RuneCraftorySpells.DOUBLE_BULLET.get() : null))
                return;
            if (command == 1)
