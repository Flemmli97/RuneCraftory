...
public class AmbrosiaWaveEntity extends BaseDamageCloud {
    ...
    public AmbrosiaWaveEntity(Level level, LivingEntity shooter, int maxLivingTick) {
        super(RuneCraftoryEntities.AMBROSIA_WAVE.get(), level, shooter);
    ...
    public void tick() {
        ...
        if (this.level().isClientSide) {
            ...
            if (this.canStartDamage())
                this.playSound(RuneCraftorySounds.ENTITY_AMBROSIA_WAVE.get(), 1, 1);
    ...
    protected boolean damageEntity(LivingEntity e) {
        DynamicDamage.Builder builder = new DynamicDamage.Builder(this, this.getOwner()).magic().noKnockback().hurtResistant(5).element(ItemElement.EARTH)
                .withChangedAttribute(RuneCraftoryAttributes.DRAIN.asHolder(), 50);
        if (CombatUtils.damageWithFaintAndCrit(this.getOwner(), e, builder, CombatUtils.getAttributeValue(this.getOwner(), RuneCraftoryAttributes.MAGIC_ATTACK.asHolder()) * this.damageMultiplier, null)) {
