...
public class RFCalculations {
		...
		if (target.canBeAttackedWithItem())
        {
            if (!target.hitByEntity(player))
            {
            		...
            		if (damagePhys > 0.0F)
                {
                    ...
                    boolean flag5 = target.attackEntityFrom(CustomDamage.doAttack(player, ItemNBT.getElement(stack), ignoreArmor?CustomDamage.DamageType.IGNOREDEF:CustomDamage.DamageType.NORMAL), damagePhys);
	...
	public static double initStatIncreaseLevel(double baseValue, int level, boolean isBoss, boolean isHealth)
	{
		...
		double newValue = baseValue+baseValue/5.0*Math.max(0, level-LibConstants.baseLevel)*scale;
		if(isHealth)
		{
	...
	{
		float scale = isBoss?1.3F:1.0F;
		double newValue = currentValue +baseValue/5.0*scale;
		if(isHealth)
		{
			int dec = (int) (newValue/LibConstants.DAMAGESCALE);
			newValue = dec*LibConstants.DAMAGESCALE;
