...
public class ItemToolHammer extends ItemPickaxe implements IRpUseItem, IItemBase, IItemWearable{
	...
	public ItemToolHammer(EnumToolTier tier) {
        ...
        this.setRegistryName(new ResourceLocation(RFReference.MODID, "hammer_" + tier.getName()));	
	...
	private void initStats()
	{
		...
		{
			case SCRAP:
				this.stats.put(IRFAttributes.RFATTACK, +3);
				this.stats.put(IRFAttributes.RFDIZ, 5);
				this.stats.put(IRFAttributes.RFCRIT, -5);
				this.stats.put(IRFAttributes.RFSTUN, 20);
				break;
			case IRON:
				this.stats.put(IRFAttributes.RFATTACK, 25);
				this.stats.put(IRFAttributes.RFDIZ, 5);
				this.stats.put(IRFAttributes.RFCRIT, -5);
				this.stats.put(IRFAttributes.RFSTUN, 20);
				break;
			case SILVER:
				this.stats.put(IRFAttributes.RFATTACK, 68);
				...
				break;
			case GOLD:
				this.stats.put(IRFAttributes.RFATTACK, 106);
				...
				break;
			case PLATINUM:
				this.stats.put(IRFAttributes.RFATTACK, 207);
				this.stats.put(IRFAttributes.RFDEFENCE, 50);
				this.stats.put(IRFAttributes.RFMAGICATT, 15);
				this.stats.put(IRFAttributes.RFMAGICDEF, 30);
				this.stats.put(IRFAttributes.RFDIZ, 18);
				this.stats.put(IRFAttributes.RFCRIT, -5);
				this.stats.put(IRFAttributes.RFSTUN, 75);
				break;
			default:
				break;
		}
			...
			for(int x = -range; x <= range;x ++)
			{
				for(int z = -range; z<= range;z++)
				{
					...
					if (player.canPlayerEdit(posNew.offset(EnumFacing.UP), EnumFacing.DOWN, itemstack))
					{
						...
						{
							for(int j = 0;j<4;j++)
							worldIn.spawnParticle(EnumParticleTypes.BLOCK_CRACK, true, posNew.getX()+0.5, posNew.getY()+1.3, posNew.getZ()+0.5, 0, (double)0.1, 0, Block.getStateId(Blocks.DIRT.getDefaultState()));
							worldIn.setBlockState(posNew, Blocks.DIRT.getDefaultState().withProperty(BlockDirt.VARIANT, BlockDirt.DirtType.DIRT), 11);
							flag=true;
						}
						//block mineral ore
					}
		...
		if(this.tier.getTierLevel()==0)
		{
	        ...
	        if(player.canPlayerEdit(pos.offset(facing), facing, itemstack))
	        {
	            ...
	            if (facing != EnumFacing.DOWN && worldIn.isAirBlock(pos.up()))
	            {
	                ...
	                {
	                		for(int j = 0;j<4;j++)
						worldIn.spawnParticle(EnumParticleTypes.BLOCK_CRACK, true, pos.getX()+0.5, pos.getY()+1.3, pos.getZ()+0.5, 0, (double)0.1, 0, Block.getStateId(Blocks.DIRT.getDefaultState()));
