	...
	public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ)
    {
		if(!worldIn.isRemote)
		{
            ...
            if (player.canPlayerEdit(pos, facing, itemstack) && player.canPlayerEdit(blockpos, facing, itemstack))
            {
                ...
                {
                    IBlockState iblockstate2 = savedBlock.getDefaultState().withProperty(BlockForge.FACING, blockFacing).withProperty(BlockForge.PART, BlockForge.EnumPartType.LEFT);
                    worldIn.setBlockState(pos, iblockstate2);
                    worldIn.setBlockState(blockpos, iblockstate2.withProperty(BlockForge.PART, BlockForge.EnumPartType.RIGHT));
