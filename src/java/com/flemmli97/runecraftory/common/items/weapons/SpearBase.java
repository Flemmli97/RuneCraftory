...
public abstract class SpearBase extends ItemSword implements IRpUseItem, IItemBase, IItemWearable{
	...
	public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft)
    {
		if(entityLiving instanceof EntityPlayer)
		{
			...
			IPlayerAnim anim = player.getCapability(PlayerCapProvider.PlayerAnim, null);
			if(anim.canUseSpear((this.getMaxItemUseDuration(stack) - timeLeft)>=this.getChargeTime()[0]))
			{
				if(!player.world.isRemote && player instanceof EntityPlayerMP)
				{
					PacketHandler.sendTo(new PacketWeaponAnimation(10), (EntityPlayerMP) player);
				}
				this.useRunePointCharge(player, 0);
				this.levelSkill(player, 0, EnumSkills.SPEAR);
				List<EntityLivingBase> entityList = RFCalculations.calculateEntitiesFromLook(player, this.getWeaponType().getRange()+2, 1);
				if(!entityList.isEmpty())
				{
			    		for (EntityLivingBase e: entityList)
			    		{
	            			...
	            			e.knockBack(player, 1, e.motionX/2, e.motionZ/2);
