...
public class RFCalculations {
	...
	public static float tamingMultiplerOnLevel(int level)
	{
		if(level<=0)
			level=1;
		float f = 1;///level;
	...
	public static boolean doPlayerDamagePhys(EntityPlayer player, EntityLivingBase target)
	{
		...
		IItemWearable item = (IItemWearable) stack.getItem();
		IRpUseItem item2 = (IRpUseItem) stack.getItem();
		if (target.canBeAttackedWithItem())
        {
            if (!target.hitByEntity(player))
            {
            		...
            		float damagePhys = cap.getStr();
            			damagePhys+=getAttributeValue(player, IRFAttributes.RFATTACK, null, null);
            		...
            		if (damagePhys > 0.0F)
                {
            			player.getCooldownTracker().setCooldown(stack.getItem(), item2.itemCoolDownTicks());
            			boolean faintChance = player.world.rand.nextInt(100)<getAttributeValue(player, IRFAttributes.RFFAINT, target, IRFAttributes.RFRESFAINT);
            			boolean critChance = player.world.rand.nextInt(100)<getAttributeValue(player, IRFAttributes.RFCRIT, target, IRFAttributes.RFRESCRIT);
            			boolean knockBackChance = player.world.rand.nextInt(100)<getAttributeValue(player, IRFAttributes.RFKNOCK, target, SharedMonsterAttributes.KNOCKBACK_RESISTANCE);
                    ...
                    boolean flag5 = target.attackEntityFrom(CustomDamage.doAttack(player, item.getElement(), ignoreArmor?1:0), damagePhys);
                    ...
                    if (flag5)
                    {
                        ...
                        float drainPercent= getAttributeValue(player, IRFAttributes.RFDRAIN, target,IRFAttributes.RFRESDRAIN);
                        if(drainPercent>0)
                        ...
                        else if(item2.getWeaponType().getAOE()==0)
                        {
                        ...
                        if (!(target instanceof IEntityBase) && item.getElement()==EnumElement.FIRE && !target.isBurning())
                        {
                            target.setFire(8);
                        }
	...
	{        
		...
		{
	        boolean poisonChance = attackingEntity.world.rand.nextInt(100)<getAttributeValue(attackingEntity, IRFAttributes.RFPOISON, target, IRFAttributes.RFRESPOISON);
	        boolean sleepChance= attackingEntity.world.rand.nextInt(100)<getAttributeValue(attackingEntity, IRFAttributes.RFSLEEP, target, IRFAttributes.RFRESSLEEP);
	        boolean fatigueChance= attackingEntity.world.rand.nextInt(100)<getAttributeValue(attackingEntity, IRFAttributes.RFFAT, target, IRFAttributes.RFRESFAT);
	        boolean coldChance= attackingEntity.world.rand.nextInt(100)<getAttributeValue(attackingEntity, IRFAttributes.RFCOLD, target, IRFAttributes.RFRESCOLD);
	        boolean paraChance= attackingEntity.world.rand.nextInt(100)<getAttributeValue(attackingEntity, IRFAttributes.RFPARA, target, IRFAttributes.RFRESPARA);
	        boolean sealChance= attackingEntity.world.rand.nextInt(100)<getAttributeValue(attackingEntity, IRFAttributes.RFSEAL, target, IRFAttributes.RFRESSEAL);
	        boolean dizzyChance= attackingEntity.world.rand.nextInt(1000)<getAttributeValue(attackingEntity, IRFAttributes.RFDIZ, target, IRFAttributes.RFRESDIZ);
	        boolean stunChance= attackingEntity.world.rand.nextInt(100)<getAttributeValue(attackingEntity, IRFAttributes.RFSTUN, target, IRFAttributes.RFRESSTUN);
	...
	public static float getAttributeValue(EntityLivingBase entity, IAttribute att, EntityLivingBase target, IAttribute resAtt)
	{
		...
		{
			IItemWearable mainHand = (IItemWearable) entity.getHeldItemMainhand().getItem();
			if(mainHand.statIncrease().get(att)!=null)
			{
				increase += mainHand.statIncrease().get(att).floatValue();
			}
			for(ItemStack equip : entity.getArmorInventoryList())
			{
				if(!equip.isEmpty() && equip.getItem() instanceof IItemWearable)
				{
					IItemWearable armor = (IItemWearable) equip.getItem();
					if(armor.statIncrease().get(att)!=null)
	        		{
						increase += armor.statIncrease().get(att).floatValue();
	        		}
				}
			}
			if(!entity.getHeldItemOffhand().isEmpty()&& entity.getHeldItemOffhand().getItem() instanceof IItemWearable && !(entity.getHeldItemOffhand().getItem() instanceof IRpUseItem))
			{
				IItemWearable offHand = (IItemWearable) entity.getHeldItemOffhand().getItem();
				if(offHand.statIncrease().get(att)!=null)
		    		{
					increase += offHand.statIncrease().get(att).floatValue();
		    		}
			}
		}
		...
		{
			if(target.getHeldItemMainhand().getItem()instanceof IItemWearable)
			{
				IItemWearable mainHand = (IItemWearable) target.getHeldItemMainhand().getItem();
				if(mainHand.statIncrease().get(att)!=null)
				{
					increase -= mainHand.statIncrease().get(att).floatValue();
				}
			...
			{
				IItemWearable offHand = (IItemWearable) target.getHeldItemOffhand().getItem();
				if(offHand.statIncrease().get(att)!=null)
		    		{
					increase -= offHand.statIncrease().get(att).floatValue();
		    		}
