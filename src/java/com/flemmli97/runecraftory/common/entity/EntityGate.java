...
public class EntityGate extends EntityLiving  implements IEntityBase{
	...
	protected void applyEntityAttributes() {
		this.getAttributeMap().registerAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(100*RFNumbers.DAMAGESCALE);;
        this.getAttributeMap().registerAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE).setBaseValue(1.0D);;
        this.getAttributeMap().registerAttribute(IRFAttributes.RFDEFENCE).setBaseValue(5);;
        this.getAttributeMap().registerAttribute(IRFAttributes.RFMAGICDEF).setBaseValue(5);;
	}
	@Override
	protected void entityInit()
    {
        ...
        this.dataManager.register(level, RFNumbers.baseLevel);
    }
	//=====NBT
	@Override
		...
		switch(this.type)
		{
			case DARK:
				while(max<2)
				{
					...
					this.entityDropItem(new ItemStack(ModItems.crystal, 1, 5), 0);
					max++;
				}
				break;
			case EARTH:
				while(max<2)
				{
					...
					this.entityDropItem(new ItemStack(ModItems.crystal, 1, 1), 0);
					max++;
				}
				break;
			case FIRE:
				...
				{
					if(this.rand.nextFloat() < 0.25+luck)
					this.entityDropItem(new ItemStack(ModItems.crystal, 1, 6), 2);
					max++;
				}
				break;
			case LIGHT:
				...
				{
					if(this.rand.nextFloat() < 0.1+luck)
					this.entityDropItem(new ItemStack(ModItems.crystal, 1, 4), 0);
					max++;
				}
				break;
			case LOVE:
				while(max<1)
				{
					...
					this.entityDropItem(new ItemStack(ModItems.crystal, 1, 6), 0);
					max++;
				}
				break;
			case NONE:
			...
			case WATER:
				while(max<2)
				{
					...
					this.entityDropItem(new ItemStack(ModItems.crystal, 1, 0), 0);
					max++;
				}
				break;
			case WIND:
				...
				{
					if(this.rand.nextFloat() < 0.3+luck)
					this.entityDropItem(new ItemStack(ModItems.crystal, 1, 3), 0);
					max++;
				}
				break;
		}
	...
	public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata) {
		...
		this.getAttributeMap().getAttributeInstance(IRFAttributes.RFDEFENCE).setBaseValue(
				RFCalculations.statIncreaseLevel((float) this.getAttributeMap().getAttributeInstance(IRFAttributes.RFDEFENCE).getBaseValue(), this.dataManager.get(level), false));
		this.getAttributeMap().getAttributeInstance(IRFAttributes.RFMAGICDEF).setBaseValue(
				RFCalculations.statIncreaseLevel((float) this.getAttributeMap().getAttributeInstance(IRFAttributes.RFMAGICDEF).getBaseValue(), this.dataManager.get(level), false));
