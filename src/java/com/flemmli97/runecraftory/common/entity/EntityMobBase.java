...
public abstract class EntityMobBase extends EntityCreature  implements IEntityAdvanced, IMob{
	...
	public EntityAINearestAttackableTarget<EntityCreature> targetMobs = new EntityAINearestAttackableTarget<EntityCreature>(this, EntityCreature.class,10,true, true, new Predicate<EntityCreature>() {
		...
		public boolean apply(EntityCreature mob) {
			return EntityMobBase.this.isTamed() && IMob.VISIBLE_MOB_SELECTOR.apply(mob)  && !(mob instanceof EntityMobBase && ((EntityMobBase) mob).isTamed());
		}} ) ;
	...
	protected void initEntityAI() {
		...
		//this.tasks.addTask(1, new EntityAIGenericAttack(this, 1, 10, 5, 1)); //TODO
	...
	protected EntityMobBase(World world, int level, boolean ridable, int baseXP, int baseMoney, boolean isFlyingEntity)
	{
		this(world);
		this.level = level;
	...
	protected void applyEntityAttributes() {
		this.getAttributeMap().registerAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(20*RFNumbers.DAMAGESCALE);;
        this.getAttributeMap().registerAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE);
        this.getAttributeMap().registerAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(0.2);;
        this.getAttributeMap().registerAttribute(SharedMonsterAttributes.FOLLOW_RANGE).setBaseValue(16.0D);
        this.getAttributeMap().registerAttribute(IRFAttributes.RFATTACK);
        this.getAttributeMap().registerAttribute(IRFAttributes.RFDEFENCE);
        this.getAttributeMap().registerAttribute(IRFAttributes.RFMAGICATT);
        this.getAttributeMap().registerAttribute(IRFAttributes.RFMAGICDEF);
	}
	...
	public void heal(float healAmount){
		healAmount*=RFNumbers.DAMAGESCALE;
	...
	protected boolean processInteract(EntityPlayer player, EnumHand hand) {
		...
		if(!stack.isEmpty() && player.isSneaking() && !this.world.isRemote)
		{
			if(!this.isTamed() && this.tamingTick==-1 && this.deathTime==0)
			{
				...
				for(Item item : this.tamingItem())
					if(item== stack.getItem())
						flag=true;
				float rightItemMultiplier = flag ? 0.6F : 1.1F;
		...
		if (!this.isEntityInvulnerable(damageSrc))
        {
            ...
            if (damageAmount <= 0) return;
            	...
            	if(damageSrc != DamageSource.OUT_OF_WORLD)
	    			damageAmount*=RFNumbers.DAMAGESCALE;
	...
	protected float reduceDamage(DamageSource damageSrc, float damageAmount)
	{
		...
		if(!damageSrc.isDamageAbsolute() && !damageSrc.isUnblockable())
			if(damageSrc.isMagicDamage())
				reduce = (float)this.getEntityAttribute(IRFAttributes.RFMAGICDEF).getAttributeValue();
			else
				reduce = (float)this.getEntityAttribute(IRFAttributes.RFDEFENCE).getAttributeValue();
	...
	public boolean attackEntityAsMob(Entity entity) {
		...
		{
			float damage = (float)this.getEntityAttribute(IRFAttributes.RFATTACK).getAttributeValue();
			boolean faintChance = this.world.rand.nextFloat()<RFCalculations.getAttributeValue(this, IRFAttributes.RFFAINT, (EntityLivingBase) entity, IRFAttributes.RFRESFAINT);
			boolean critChance = this.world.rand.nextFloat()<RFCalculations.getAttributeValue(this, IRFAttributes.RFCRIT, (EntityLivingBase) entity, IRFAttributes.RFRESCRIT);
			boolean knockBackChance = this.world.rand.nextFloat()<RFCalculations.getAttributeValue(this, IRFAttributes.RFKNOCK, (EntityLivingBase) entity, SharedMonsterAttributes.KNOCKBACK_RESISTANCE);
	        ...
	        if (flag)
	        {
	            ...
	            float drainPercent= RFCalculations.getAttributeValue(this, IRFAttributes.RFDRAIN, (EntityLivingBase) entity,IRFAttributes.RFRESDRAIN);
                if(drainPercent>0)
	...
	protected void dropLoot(boolean wasRecentlyHit, int lootingModifier, DamageSource source) {
		...
		for(Item item : this.getDrops().keySet())
		{
			int max = 0;
			float luck =0;
			if(source.getTrueSource() instanceof EntityPlayer)
				luck = RFCalculations.getAttributeValue((EntityLivingBase) source.getTrueSource(), IRFAttributes.RFRLUCK, null, null);
			while(luck>=1.0F)
			{
				luck-=0.8;
				max-=1;
			}
			while(this.rand.nextFloat() < this.getDrops().get(item)+luck && max<2)
			{
	...
	public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, IEntityLivingData livingdata) {
		...
		this.getAttributeMap().getAttributeInstance(IRFAttributes.RFATTACK).setBaseValue(
				RFCalculations.statIncreaseLevel((float) this.getAttributeMap().getAttributeInstance(IRFAttributes.RFATTACK).getBaseValue(), level, false));
		this.getAttributeMap().getAttributeInstance(IRFAttributes.RFDEFENCE).setBaseValue(
				RFCalculations.statIncreaseLevel((float) this.getAttributeMap().getAttributeInstance(IRFAttributes.RFDEFENCE).getBaseValue(), level, false));
		this.getAttributeMap().getAttributeInstance(IRFAttributes.RFMAGICATT).setBaseValue(
				RFCalculations.statIncreaseLevel((float) this.getAttributeMap().getAttributeInstance(IRFAttributes.RFMAGICATT).getBaseValue(), level, false));
		this.getAttributeMap().getAttributeInstance(IRFAttributes.RFMAGICDEF).setBaseValue(
				RFCalculations.statIncreaseLevel((float) this.getAttributeMap().getAttributeInstance(IRFAttributes.RFMAGICDEF).getBaseValue(), level, false));
