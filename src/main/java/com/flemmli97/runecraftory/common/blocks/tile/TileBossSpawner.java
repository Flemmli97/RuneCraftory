...
public class TileBossSpawner extends TileEntity implements ITickable{
	...
	{
		Entity entity = GateSpawning.entityFromString(world, savedEntity, RFCalculations.levelFromDistSpawn(world, pos));
        int k = world.getEntitiesWithinAABB(entity.getClass(), (new AxisAlignedBB(this.pos).grow(32))).size();
        if(k!=0)
        {        
        		return;
        }
        EntityLiving mob = entity instanceof EntityLiving ? (EntityLiving)entity : null;
