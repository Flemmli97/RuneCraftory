...
public abstract class EntityMobBase extends EntityCreature  implements IEntityAdvanced, IMob, IEntityAdditionalSpawnData{
	...
	public EntityMobBase(World world) {
		super(world);
		this.prop=EntityDefaultStats.getDefaultStats(this.getClass());
		if(this.prop==null)
		...
		this.genes.put(ItemStats.RFATTACK, 1.1F-world.rand.nextFloat()*0.2F);
		this.genes.put(ItemStats.RFDEFENCE, 1.1F-world.rand.nextFloat()*0.2F);
		this.genes.put(ItemStats.RFMAGICATT, 1.1F-world.rand.nextFloat()*0.2F);
		this.genes.put(ItemStats.RFMAGICDEF, 1.1F-world.rand.nextFloat()*0.2F);
	...
	private void onLevelUp()
	{
		this.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(
				LevelCalc.onEntityLevelUp(this.prop.getBaseValues().get(SharedMonsterAttributes.MAX_HEALTH), this.getAttributeMap().getAttributeInstance(SharedMonsterAttributes.MAX_HEALTH).getAttributeValue(), false, true));
		this.getAttributeMap().getAttributeInstance(ItemStats.RFATTACK).setBaseValue(
				LevelCalc.onEntityLevelUp(this.prop.getBaseValues().get(ItemStats.RFATTACK), this.getAttributeMap().getAttributeInstance(ItemStats.RFATTACK).getAttributeValue(), false, false));
		this.getAttributeMap().getAttributeInstance(ItemStats.RFDEFENCE).setBaseValue(
				LevelCalc.onEntityLevelUp(this.prop.getBaseValues().get(ItemStats.RFDEFENCE), this.getAttributeMap().getAttributeInstance(ItemStats.RFDEFENCE).getAttributeValue(), false, false));
		this.getAttributeMap().getAttributeInstance(ItemStats.RFMAGICATT).setBaseValue(
				LevelCalc.onEntityLevelUp(this.prop.getBaseValues().get(ItemStats.RFMAGICATT), this.getAttributeMap().getAttributeInstance(ItemStats.RFMAGICATT).getAttributeValue(), false, false));
		this.getAttributeMap().getAttributeInstance(ItemStats.RFMAGICDEF).setBaseValue(
				LevelCalc.onEntityLevelUp(this.prop.getBaseValues().get(ItemStats.RFMAGICDEF), this.getAttributeMap().getAttributeInstance(ItemStats.RFMAGICDEF).getAttributeValue(), false, false));
    ...
    public void notifyDataManagerChange(DataParameter<?> key)
	{
		if(key.equals(level))
		{
			if(this.prop.getBaseValues()!=null)
			{
				this.updateStatsToLevel();
			}
		}
	...
	public ItemStack[] tamingItem() {
		return tamingItem.toArray(new ItemStack[0]);
	}
	...
	public void writeEntityToNBT(NBTTagCompound compound) {
		...
		{
			nbtEffects.appendTag(new NBTTagString(effect.getName()));
		}
		compound.setTag("Effects", nbtEffects);
		NBTTagCompound nbtGene = new NBTTagCompound();
		for(IAttribute att : this.genes.keySet())
		{
			nbtGene.setFloat(att.getName(), this.genes.get(att));
		}
	...
	public void readEntityFromNBT(NBTTagCompound compound) {
		...
		if(compound.hasKey("MobLevel"))
			this.dataManager.set(level, compound.getInteger("MobLevel"));
		if(compound.hasKey("Tamed"))
			this.dataManager.set(isTamed, compound.getBoolean("Tamed")); 
		NBTTagList nbtEffects = compound.getTagList("Effects", Constants.NBT.TAG_STRING);
		if(nbtEffects!=null)
			nbtEffects.forEach(tag -> this.activeEffects.add(EnumStatusEffect.fromName(((NBTTagString)tag).getString())));
		...
		for(String s : nbtGenes.getKeySet())
		{
			...
			if(s.equals(SharedMonsterAttributes.MAX_HEALTH.getName()))
				...
				att = ItemStats.ATTRIBUTESTRINGMAP.get(s);
	...
	private void updateStatsToLevel()
	{
		...
		{
			double leveledValue = LevelCalc.initStatIncreaseLevel(this.prop.getBaseValues().get(att), this.level(), this instanceof EntityBossBase, att.equals(SharedMonsterAttributes.MAX_HEALTH));
