...
public class ItemShortSwordBase extends SwordItem implements IItemUsable, IChargeable, IAOEWeapon {
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World world, LivingEntity entity, int timeLeft) {
        if (!world.isRemote && this.getUseDuration(stack) - timeLeft >= this.getChargeTime(stack)) {
            List<LivingEntity> list = RayTraceUtils.getEntities(entity, this.getRange()+2, this.getFOV()+10);
            if (!list.isEmpty()) {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand) {
        ...
        if (player.isCreative() || player.getCapability(PlayerCapProvider.PlayerCap).map(cap->cap.getSkillLevel(EnumSkills.SHORTSWORD)[0]>=5).orElse(false)) {
