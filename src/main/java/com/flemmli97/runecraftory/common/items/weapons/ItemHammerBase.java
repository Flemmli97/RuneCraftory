...
public class ItemHammerBase extends ItemPickaxe implements IItemUsable, IChargeable, IAOEWeapon
{
    private int chargeXP = 25;
    
    public ItemHammerBase(String name) {
        super(ModItems.mat);
    ...
    public boolean onBlockDestroyed(ItemStack stack, World world, IBlockState state, BlockPos pos, EntityLivingBase entityLiving) {
        if (!world.isRemote && entityLiving instanceof EntityPlayer && state.getBlock() instanceof BlockMineral) {
    ...
    public void levelSkillOnBreak(EntityPlayer player) {
        IPlayer cap = player.getCapability(PlayerCapProvider.PlayerCap, null);
        cap.increaseSkill(EnumSkills.MINING, player, 1);
    }
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft) {
        if (entityLiving instanceof EntityPlayer) {
            ...
            if (this.getMaxItemUseDuration(stack) - timeLeft >= this.getChargeTime()[0]) {
                ...
                List<EntityLivingBase> entityList = RayTraceUtils.getEntities(player, this.getRange(), 360);
