...
public abstract class SpearBase extends ItemSword implements IItemUsable, IModelRegister, IChargeable
{
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft) {
        if (entityLiving instanceof EntityPlayer) {
            ...
            if (this.getMaxItemUseDuration(stack) - timeLeft >= this.getChargeTime()[0]) {
                anim.startSpear();
                (player.getCapability(CapabilityProvider.PlayerCapProvider.PlayerCap, null)).decreaseRunePoints(player, 10);
                this.useSpear(player, stack);
            }
            else if (anim.canUseSpear()) {
                this.useSpear(player, stack);
            }
        }
    }
    ...
    private void useSpear(EntityPlayer player, ItemStack stack) {
        ...
        List<EntityLivingBase> entityList = RFCalculations.calculateEntitiesFromLook(player, this.getWeaponType().getRange(), 1.0f);
