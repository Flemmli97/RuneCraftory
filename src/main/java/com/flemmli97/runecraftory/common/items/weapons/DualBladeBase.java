...
public abstract class DualBladeBase extends ItemSword implements IItemUsable, IModelRegister, IChargeable
{
    private int chargeXP;
    
    public DualBladeBase(String name) {
        super(ModItems.mat);
        this.chargeXP = 25;
    ...
    public void levelSkillOnHit(EntityPlayer player) {
        ...
        cap.increaseSkill(EnumSkills.DUAL, player, 1);
    }
    ...
    public void onPlayerStoppedUsing(ItemStack stack, World worldIn, EntityLivingBase entityLiving, int timeLeft) {
        if (entityLiving instanceof EntityPlayer) 
        {
            ...
            if (this.getMaxItemUseDuration(stack) - timeLeft >= this.getChargeTime()[0]) 
            {
                ...
                List<EntityLivingBase> entityList = RFCalculations.calculateEntitiesFromLook(player, this.getWeaponType().getRange() + 2.0f, this.getWeaponType().getAOE());
