...
public class EntityAmbrosiaWave extends Entity implements IOwnable<EntityAmbrosia> {
    ...
    public void tick() {
        ...
        if (this.world.isRemote) {
            if(this.livingTick < this.dataManager.get(maxTick) && this.livingTick % 5 == 1) {
                for(Vector3f vec : circleParticleMotion) {
                    this.world.addParticle(new ColoredParticleData(ModParticles.ambrosiaWaveParticle.get(), 219/255F, 153/255F, 66/255F, 1, 0.4f), this.getX(), this.getY()+0.2, this.getZ(), vec.getX(), vec.getY(), vec.getZ());
                }
            }
        }
        if (!this.world.isRemote) {
